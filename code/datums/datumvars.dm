...
/client/proc/debug_variable(name, value, level, datum/DA = null)
	...
	if (isnull(value))
		html += "[lhtml_encode(name)] = <span class='value'>null</span>"

	else if (istext(value))
		html += "[lhtml_encode(name)] = <span class='value'>\"[lhtml_encode(value)]\"</span>"
		...
		html += "[lhtml_encode(name)] = (<span class='value'>[value]</span>) <img class=icon src=\"[rname]\">"
		#else
		html += "[lhtml_encode(name)] = /icon (<span class='value'>[value]</span>)"
		#endif
...
*/
		...
		html += "[lhtml_encode(name)] = <span class='value'>'[value]'</span>"

	else if (istype(value, /datum))
		var/datum/D = value
		html += "<a href='?_src_=vars;Vars=\ref[value]'>[lhtml_encode(name)] \ref[value]</a> = [D.type]"

	else if (istype(value, /client))
		var/client/C = value
		html += "<a href='?_src_=vars;Vars=\ref[value]'>[lhtml_encode(name)] \ref[value]</a> = [C] [C.type]"
//
	else if (istype(value, /list))
		var/list/L = value
		html += "[lhtml_encode(name)] = /list ([L.len])"
