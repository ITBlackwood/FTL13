		...
		if(src.allowed(usr))
			...
			if(can_control(usr, R))
				...
				if(choice == "Confirm" && can_control(usr, R) && !..())
					...
					if(R.connected_ai)
						R.connected_ai << "[!R.lockcharge ? "<span class='notice'>NOTICE - Cyborg lockdown lifted" : "<span class='alert'>ALERT - Cyborg lockdown detected"]: <a href='?src=\ref[R.connected_ai];track=[lhtml_encode(R.name)]'>[R.name]</a></span><br>"
