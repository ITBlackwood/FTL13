...
/datum/admins/proc/manage_free_slots()
	...
	if(SSjob)
		for(var/datum/job/job in SSjob.occupations)
			count++
			var/J_title = lhtml_encode(job.title)
			var/J_opPos = lhtml_encode(job.total_positions - (job.total_positions - job.current_positions))
			var/J_totPos = lhtml_encode(job.total_positions)
